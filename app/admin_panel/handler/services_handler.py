...
async def city(call: CallbackQuery, state: FSMContext):
    ...
    if call.data.isdigit():
        ...
        await call.message.edit_text("Выберите нужный сервис", reply_markup=get_services_kb())
...
async def add_new_city(message: Message | CallbackQuery, state: FSMContext):
    ...
    if isinstance(message, CallbackQuery):
        await message.message.edit_text("Выберите город или другое действие", reply_markup=city_kb())
        await FSMService.city.set()
    else:
        add_new_city_in_db(city_name=message.text)
        await message.answer("🟢 Город добавлен\n"
                             "Выберите город или другое действие", reply_markup=city_kb())
        async with state.proxy() as data:
            msg = data['msg']
        await msg.delete()
        await FSMService.city.set()
...
async def services(call: CallbackQuery, state: FSMContext):
    ...
    if call.data.isdigit():
        ...
        await call.message.edit_text("Выберите тип услуги", reply_markup=get_type_service_kb(service_id=int(call.data)))
        ...
        await call.message.edit_text("Выберите сервис для удаления", reply_markup=remove_services_kb())
...
async def add_new_service(message: Message | CallbackQuery, state: FSMContext):
    ...
    if isinstance(message, CallbackQuery):
        await message.message.edit_text("Выберите нужный сервис", reply_markup=get_services_kb())
        await FSMService.services.set()
    else:
        add_new_service_in_db(service_name=message.text)
        await message.answer("🟢 Сервис добавлен\n"
                             "Выберите нужный сервис", reply_markup=get_services_kb())
        await FSMService.services.set()
...
async def add_new_type_service(message: Message | CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        service_id = data['service_id']
    if isinstance(message, CallbackQuery):
        await message.message.edit_text("Выберите тип услуги", reply_markup=get_type_service_kb(service_id=service_id))
        await FSMService.type_service.set()
    else:
